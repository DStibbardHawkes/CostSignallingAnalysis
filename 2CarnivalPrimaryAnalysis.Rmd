...
First we'll start with a mean only model, followed by a model including game-type only. We're going to use zero-centred weak regularizing priors throughout. Regularizing priors usually result in better out of sample fit, as simulated in section 7.3 of rethinking. As an added bonus, they also stop BRMs warning of insufficient MCMC samples.
...
CrGtFit <- fitted(CrGt, newdata = nd, probs = c(.05,.95)) %>% 
  ...
  ggplot(aes(x = Estimate, y = new)) + geom_point() + geom_errorbar(aes(xmin = Q5, xmax = Q95),width=0, lwd = 1)
...
CrGtrS <- brm(data = d,
              family = bernoulli,
              Played1 ~ 0 + Condition + (0 + Condition | Game_Type) + (0 + Condition | sex),
              prior = c(prior(normal(0, 1), class = b)
                        ,prior(exponential(1), class = sd, group = Game_Type)
                        ,prior(lkj(2), class = cor, group = Game_Type)
...
Yes, including participant gender substantially improves the model.

And let's inspect the fitted values
...
CrGtrSFitM <- fitted(CrGtrS, newdata = nd, probs = c(.05,.95)) %>% 
  ...
  filter(sex == "M") %>%
  unite(col = "new", c(Condition, Game_Type), sep = "_") %>%
  ggplot(aes(x = Estimate, y = new)) + geom_point(color = "blue") + geom_errorbar(aes(xmin = Q5, xmax = Q95),width=0, lwd = 1,  color = "blue") 
...
CrGtrSFitF <- fitted(CrGtrS, newdata = nd, probs = c(.05,.95)) %>% 
  ...
  filter(sex == "F") %>%
  unite(col = "new", c(Condition, Game_Type), sep = "_") %>%
  ggplot(aes(x = Estimate, y = new)) + geom_errorbar(aes(xmin = Q5, xmax = Q95), color = "hotpink", width=0, lwd = 1) + geom_point(color = "hotpink")  + theme(axis.title = element_blank(), axis.text.y=element_blank())
...
We're also interested in the impacts of age. Younger people potentially have more to gain by showing off. Let's see how it performs both on its own and alongside gender. 

First let's look at age on its own. In the first model we'll use age without binning. As we're only interested in broad trends, rather than year-by-year changes, we'll also code age as a binary variable in the 2nd model using median age split (i.e., 37 years).
...
A <- add_criterion(A, criterion = c("loo", "waic"))
...
BinaryA <- add_criterion(BinaryA, criterion = c("loo", "waic"))

```


Next let's plot change in play probability with increasing age, with all other variables excluded.

```{r}
Age_seq <- 
  tibble(age = seq(from = 17, to = 75, length.out = 59))

fitd_play <-
  fitted(A, newdata = Age_seq, probs = c(.05,.95)) %>%
  data.frame() %>%
  bind_cols(Age_seq)
...
fig4 <-
  ggplot(data = d, 
       aes(x = age)) +
  geom_smooth(data = fitd_play,
              aes(y = Estimate, ymin = Q5, ymax = Q95),
              stat = "identity",
              fill = "grey70", color = "black", alpha = 1, size = 1/2) +
 geom_point(aes(y = Played1),
             color = "navyblue", 
             shape = 1,
             size = 1.5, alpha = 1/2) +
  ylab("PlayProbability") +
...
Finally let's compare how much information gain age gives us 

```{r}
msA <- loo_compare(A, BinaryA, criterion = "loo") %>%
  round(digits = 2) %>% 
  as.data.frame() %>% 
  select(elpd_diff,se_diff) %>% 
  rownames_to_column() %>%
...
msA <- model_weights(A, BinaryA, weights = "loo") %>% 
  round(digits = 2) %>%
  as.data.frame() %>% 
  rownames_to_column() %>%
  arrange(rowname) %>%
...
CrGtrA <- brm(data = d,
              family = bernoulli,
              Played1 ~ 0 + Condition + (0 + Condition | Game_Type) + (0 + Condition | AgeSplit),
              prior = c(prior(normal(0, 1), class = b)
                        ,prior(exponential(1), class = sd, group = Game_Type)
                        ,prior(lkj(2), class = cor, group = Game_Type)
...
ms4 <- loo_compare(Baseline, GameType, CrGt, CrGtrS,CrGtrA,CrGtrSrA, criterion = "loo") %>%
  round(digits = 2) %>% 
  as.data.frame() %>% 
  select(elpd_diff,se_diff) %>% 
  rownames_to_column() %>%
...
ms4 <- model_weights(Baseline, GameType, CrGt, CrGtrS,CrGtrA,CrGtrSrA, weights = "loo") %>% 
  round(digits = 2) %>%
  as.data.frame() %>% 
  rownames_to_column() %>%
  arrange(rowname) %>%
  ...
  relocate(Weights, .after = "SE Difference") %>% 
  mutate(
    Definition = c("0 + Condition + (0 + Condition | GameType) + (0 + Condition | Sex) + (0 + Condition | Age)"
                   ,"0 + Condition + (0 + Condition | GameType) + (0 + Condition | Sex)"
                   ,"0 + Condition + (0 + Condition | GameType) + (0 + Condition | Age)"
...
ModelSelectionTable <- ms4 %>% 
  xtable(label = "Tab:LooMS",
         caption = "Leave-one-out model selection results including expected log-predictive density differences, standard errors and Akaike weights. Left side model definitions provided in BRMs syntax"
         )
...
Although the model including gender is allotted a small proportion of model weight, the model including age and gender is the best performing model by a substantial margin. Let's once again use fitted to visualise model predictions. 
...
CrGtrAFitM <- fitted(CrGtrSrA, newdata = nd, probs = c(.05,.95)) %>% 
  ...
  theme(legend.position = "none") + scale_y_discrete(labels= c("P0B1F","P0B1H", "P0B0F", "P0B0H", "P1B1F","P1B1H", "P1B0F", "P1B0H"))
...
That being the case, we'll explore this model a little further in document three. 
...
```{r}
save(list = c("CrGtrSrA", "GameType", "CrGt", "CrGtrSrA"), file = "carnivalmodels.rda") 
